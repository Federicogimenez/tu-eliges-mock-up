...
export default function ButtonPrimary (
  { 
    ...
    src, 
    customStyle, 
    fromColor = 'from-blue-gradient-start', 
    toColor = 'to-blue-gradient-end',
    fromColor2 = 'from-blue-gradient-end',
    toColor2 = 'to-blue-gradient-start',
    }: ButtonPrimaryProps) {
  ...
  return (
    <a href={src} 
      ...
      className={`group block font-semibold text-white relative cursor-pointer w-full max-w-md rounded-full text-sm md:text-lg px-8 py-4 text-center bg-white border-2 border-white shadow-lg shadow-black/50 overflow-hidden ${customStyle}`}>
        ...
        <span className='relative transition-all duration-500 group-hover:font-bold !text-white  group-hover:opacity-0'>
            {text_1}
        </span>
        <picture className={` absolute right-0 top-1/2 w-7  stroke-white dark:stroke-white scale-100 -translate-x-1/3 -translate-y-1/2 transition-all duration-300 group-hover:scale-105 group-hover:translate-x-0 `}>
