...
### Language Rule
- Documentacion interna (`.claude/`) en espanol
- Codigo y contenido user-facing en ingles

### Roles

| Role | Scope | Output location | How to activate |
|------|-------|-----------------|-----------------|
| **Architect** | Analyzes requirements, designs features, creates phased development plans. Never writes `src/` code. | `.claude/features/{feature}/` | Lee `.claude/roles/architect.md` |
| **Dev** | Implements tasks from the Architect's plans with focused, file-specific context. Never makes architectural decisions. | `src/` | Lee `.claude/roles/dev.md` + la tarea especifica de `tasks.md` |
...
| `task-spec.md` | Architect | Define a single task (one section per task inside `tasks.md`) |
...
.claude/
  ...
  features/
    business/              ← B2B landing page feature (active — 2 QA tasks pending)
    refactorize/           ← Main.tsx refactor (completed)
    seo/                   ← SEO layer (suspended — requires redo)
  standards/
...
- **Route arrays** in `src/shared/routes.ts` control layout behavior (`HERO_PAGES`, `NO_FAQS_PAGES`, `DARK_BG_PAGES`)
- **`useRouteConfig` hook** derives layout flags from route arrays + theme
